...
# Checkout the PREPROD branch and pull the latest changes
...
echo -e "${GREEN}Merging 'preprod' branch into '$current_branch'...${NC}"
git merge sit
...
echo -e "${YELLOW} Creating PR from '$current_branch' branch to 'preprod' branch.${NC}"

# Create a PR from the current branch to the preprod branch
PR_RESPONSE=$(curl -s -H "Authorization: token $GITHUB_CICD_POC_TOKEN" \
  -X POST \
  -H "Accept: application/vnd.github.v3+json" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Merge '"$current_branch"' into preprod",
    "head": "'"$current_branch"'",
    "base": "preprod",
    "body": "Auto-generated pull request to merge '"$current_branch"' into preprod"
  }' \
...
if [[ $PR_URL != "null" ]]; then
  echo -e "${YELLOW}Pull request created successfully: $PR_URL${NC}"
else
